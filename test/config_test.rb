...
class ConfigTest < Minitest::Test
  ...
  def test_resolve_precedence_cli_over_front_matter_over_file
    Dir.mktmpdir do |dir|
      ...
      cfg = Md2pdf::Config.resolve(md, font_size: '20pt')
      assert_equal '20pt', cfg[:font_size]  # CLI wins
      assert_equal 1.5, cfg[:line_height]   # from file config
    end
  end
  ...
  def test_find_config_file_walks_up_to_parent
    Dir.mktmpdir do |dir|
      ...
      assert_equal cfg, Md2pdf::Config.find_config_file(sub)
    end
  end

  def test_find_config_file_nil_when_absent
    Dir.mktmpdir do |dir|
      assert_nil Md2pdf::Config.find_config_file(dir)
    end
  end
end
